...
private[rapids] trait RapidsConfSqlEntries extends RapidsConfResourceEntries {
  ...
  val SQL_ENABLED = conf("spark.cudf.sql.enabled")
  ...
  val RANGE_PARTITIONING_SAMPLE_KEYS_ONLY =
    conf("spark.cudf.sql.rangePartitioning.sampleKeysOnly")
  ...
  val SQL_MODE = conf("spark.cudf.sql.mode")
    .doc("Set the mode for the cuDF plugin. The supported modes are explainOnly and " +
         ...
         "be changed by setting spark.cudf.sql.explain. See that config for more details.")
    .startupOnly()
    .stringConf
  ...
  val UDF_COMPILER_ENABLED = conf("spark.cudf.sql.udfCompiler.enabled")
    .doc("When set to true, Scala UDFs will be considered for compilation as Catalyst expressions")
    .commonlyUsed()
    .booleanConf
    .createWithDefault(false)

  val DFUDF_ENABLED = conf("spark.cudf.sql.dfudf.enabled")
    .doc("When set to false, the DataFrame UDF plugin is disabled. True enables it.")
    .internal()
    .booleanConf
    .createWithDefault(true)

  val INCOMPATIBLE_OPS = conf("spark.cudf.sql.incompatibleOps.enabled")
  ...
  val INCOMPATIBLE_DATE_FORMATS = conf("spark.cudf.sql.incompatibleDateFormats.enabled")
  ...
  val IMPROVED_FLOAT_OPS = conf("spark.cudf.sql.improvedFloatOps.enabled")
  ...
  val NEED_DECIMAL_OVERFLOW_GUARANTEES = conf("spark.cudf.sql.decimalOverflowGuarantees")
  ...
  val ENABLE_WINDOW_COLLECT_LIST = conf("spark.cudf.sql.window.collectList.enabled")
  ...
  val ENABLE_WINDOW_COLLECT_SET = conf("spark.cudf.sql.window.collectSet.enabled")
  ...
  val ENABLE_WINDOW_UNBOUNDED_AGG = conf("spark.cudf.sql.window.unboundedAgg.enabled")
  ...
  val ENABLE_WINDOW_GROUP_LIMIT_OPT = conf("spark.cudf.sql.window.groupLimit.opt.enabled")
  ...
  val ENABLE_FLOAT_AGG = conf("spark.cudf.sql.variableFloatAgg.enabled")
  ...
  val ENABLE_REPLACE_SORTMERGEJOIN = conf("spark.cudf.sql.replaceSortMergeJoin.enabled")
    .doc("Allow replacing sortMergeJoin with HashJoin")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_HASH_OPTIMIZE_SORT = conf("spark.cudf.sql.hashOptimizeSort.enabled")
  ...
  val ENABLE_FOLD_LOCAL_AGGREGATE = conf("spark.cudf.sql.foldLocalAggregate.enabled")
    .doc("Whether to fold two-stages local aggregate into one-shot Complete aggregate.")
    .internal()
    .booleanConf
    .createWithDefault(true)

  val ENABLE_CAST_FLOAT_TO_DECIMAL = conf("spark.cudf.sql.castFloatToDecimal.enabled")
  ...
  val ENABLE_CAST_FLOAT_TO_STRING = conf("spark.cudf.sql.castFloatToString.enabled")
  ...
  val ENABLE_FLOAT_FORMAT_NUMBER = conf("spark.cudf.sql.formatNumberFloat.enabled")
  ...
  val ENABLE_CAST_FLOAT_TO_INTEGRAL_TYPES =
    conf("spark.cudf.sql.castFloatToIntegralTypes.enabled")
  ...
  val ENABLE_CAST_DECIMAL_TO_FLOAT = conf("spark.cudf.sql.castDecimalToFloat.enabled")
      .doc("Casting from decimal to floating point types on the GPU returns results that have " +
          "tiny difference compared to results returned from CPU.")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_CAST_STRING_TO_FLOAT = conf("spark.cudf.sql.castStringToFloat.enabled")
  ...
  val ENABLE_CAST_STRING_TO_TIMESTAMP = conf("spark.cudf.sql.castStringToTimestamp.enabled")
    .doc("When set to false, this disables casting from string to timestamp on the GPU.")
    .booleanConf
    .createWithDefault(true)

  val HAS_EXTENDED_YEAR_VALUES = conf("spark.cudf.sql.hasExtendedYearValues")
  ...
  val ENABLE_INNER_JOIN = conf("spark.cudf.sql.join.inner.enabled")
      .doc("When set to true inner joins are enabled on the GPU")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_CROSS_JOIN = conf("spark.cudf.sql.join.cross.enabled")
      .doc("When set to true cross joins are enabled on the GPU")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_LEFT_OUTER_JOIN = conf("spark.cudf.sql.join.leftOuter.enabled")
      .doc("When set to true left outer joins are enabled on the GPU")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_RIGHT_OUTER_JOIN = conf("spark.cudf.sql.join.rightOuter.enabled")
      .doc("When set to true right outer joins are enabled on the GPU")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_FULL_OUTER_JOIN = conf("spark.cudf.sql.join.fullOuter.enabled")
      .doc("When set to true full outer joins are enabled on the GPU")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_LEFT_SEMI_JOIN = conf("spark.cudf.sql.join.leftSemi.enabled")
      .doc("When set to true left semi joins are enabled on the GPU")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_LEFT_ANTI_JOIN = conf("spark.cudf.sql.join.leftAnti.enabled")
      .doc("When set to true left anti joins are enabled on the GPU")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_EXISTENCE_JOIN = conf("spark.cudf.sql.join.existence.enabled")
      .doc("When set to true existence joins are enabled on the GPU")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_PROJECT_AST = conf("spark.cudf.sql.projectAstEnabled")
      .doc("Enable project operations to use cudf AST expressions when possible.")
      .internal()
      .booleanConf
      .createWithDefault(false)

  val ENABLE_TIERED_PROJECT = conf("spark.cudf.sql.tiered.project.enabled")
      .doc("Enable tiered projections.")
      .internal()
      .booleanConf
      .createWithDefault(true)

  val ENABLE_COMBINED_EXPR_PREFIX = "spark.cudf.sql.expression.combined."

  val ENABLE_COMBINED_EXPRESSIONS = conf("spark.cudf.sql.combined.expressions.enabled")
  ...
  val ENABLE_RLIKE_REGEX_REWRITE = conf("spark.cudf.sql.rLikeRegexRewrite.enabled")
  ...
  // FILE FORMATS
  val MULTITHREAD_READ_NUM_THREADS = conf("spark.cudf.sql.multiThreadedRead.numThreads")
      .doc("The maximum number of threads on each executor to use for reading small " +
        ...
        "spark.cudf.sql.format.parquet.reader.type, " +
        "spark.cudf.sql.format.orc.reader.type, or " +
        "spark.cudf.sql.format.avro.reader.type for a discussion of reader types. " +
  ...
  val MULTITHREAD_READ_MEMORY_LIMIT_ENABLED = {
    ...
    conf("spark.cudf.sql.multiThreadedRead.memoryLimit.enabled")
  ...
  val MULTITHREAD_READ_MEMORY_LIMIT_SIZE = {
    // TODO: This conf should be adjusted during the runtime
    conf("spark.cudf.sql.multiThreadedRead.memoryLimit.size")
  ...
  val MULTITHREAD_READ_MEMORY_LIMIT_ACQUISITION_TIMEOUT  = {
    conf("spark.cudf.sql.multiThreadedRead.memoryLimit.acquisitionTimeout")
  ...
  val MULTITHREAD_READ_MEMORY_LIMIT_TEST_PER_STAGE_POOL = {
    conf("spark.cudf.sql.multiThreadedRead.memoryLimit.tests.perStagePool")
  ...
  val ENABLE_PARQUET = conf("spark.cudf.sql.format.parquet.enabled")
    .doc("When set to false disables all parquet input and output acceleration")
    .booleanConf
    .createWithDefault(true)

  val ACCELERATED_COLUMNAR_TO_ROW_ENABLED =
    conf("spark.cudf.sql.acceleratedColumnarToRow.enabled")
  ...
  val ENABLE_PARQUET_INT96_WRITE = conf("spark.cudf.sql.format.parquet.writer.int96.enabled")
  ...
  val PARQUET_WRITER_ROW_GROUP_SIZE_ROWS =
    conf("spark.cudf.sql.format.parquet.writer.rowGroupSizeRows")
  ...
  val PARQUET_WRITER_ROW_GROUP_SIZE_BYTES =
    conf("spark.cudf.sql.format.parquet.writer.rowGroupSizeBytes")
  ...
  val PARQUET_WRITER_DICTIONARY_POLICY =
    conf("spark.cudf.sql.format.parquet.writer.dictionaryPolicy")
      .doc("Dictionary policy for the GPU Parquet writer. NEVER disables dictionary " +
        ...
        "spark.cudf.sql.format.parquet.writer.maxDictionarySize. ALWAYS allows " +
  ...
  val PARQUET_WRITER_MAX_DICTIONARY_SIZE =
    conf("spark.cudf.sql.format.parquet.writer.maxDictionarySize")
      .doc("Maximum size in bytes of a per-column-chunk dictionary in a Parquet row " +
        ...
        "ADAPTIVE (see spark.cudf.sql.format.parquet.writer.dictionaryPolicy); column " +
  ...
  val PARQUET_READER_FOOTER_TYPE =
    conf("spark.cudf.sql.format.parquet.reader.footer.type")
  ...
  val ENABLE_CPU_BASED_UDF = conf("spark.cudf.sql.rowBasedUDF.enabled")
  ...
  val PARQUET_READER_TYPE = conf("spark.cudf.sql.format.parquet.reader.type")
    .doc("Sets the Parquet reader type. We support different types that are optimized for " +
      ...
      "spark.cudf.sql.format.parquet.multiThreadedRead.maxNumFilesParallel to control " +
      ...
      "in the cloud. See spark.cudf.cloudSchemes.")
    .stringConf
    .transform(_.toUpperCase(java.util.Locale.ROOT))
  ...
  val PARQUET_DECOMPRESS_CPU =
    conf("spark.cudf.sql.format.parquet.decompressCpu")
      .doc("If true then the CPU is eligible to decompress Parquet data rather than the GPU. " +
          s"See other spark.cudf.sql.format.parquet.decompressCpu.* configuration settings " +
  ...
  val PARQUET_DECOMPRESS_CPU_SNAPPY =
    conf("spark.cudf.sql.format.parquet.decompressCpu.snappy")
  ...
  val PARQUET_DECOMPRESS_CPU_ZSTD =
    conf("spark.cudf.sql.format.parquet.decompressCpu.zstd")
      .doc(s"If true and $PARQUET_DECOMPRESS_CPU is true then the CPU decompresses " +
  ...
  val READER_MULTITHREADED_COMBINE_THRESHOLD =
    conf("spark.cudf.sql.reader.multithreaded.combine.sizeBytes")
      .doc("The target size in bytes to combine multiple small files together when using the " +
        ...
        "spark.cudf.sql.reader.multithreaded.combine.waitTime together, up to this " +
        ...
        "spark.cudf.sql.reader.batchSizeRows or spark.cudf.sql.reader.batchSizeBytes limits.")
      .bytesConf(ByteUnit.BYTE)
      .createWithDefault(64 * 1024 * 1024) // 64M

  val READER_MULTITHREADED_COMBINE_WAIT_TIME =
    conf("spark.cudf.sql.reader.multithreaded.combine.waitTime")
  ...
  val READER_MULTITHREADED_READ_KEEP_ORDER =
    conf("spark.cudf.sql.reader.multithreaded.read.keepOrder")
  ...
  val PARQUET_MULTITHREADED_COMBINE_THRESHOLD =
    conf("spark.cudf.sql.format.parquet.multithreaded.combine.sizeBytes")
      .doc("The target size in bytes to combine multiple small files together when using the " +
        ...
        "spark.cudf.sql.format.parquet.multithreaded.combine.waitTime together, up to this " +
        "threshold size, are combined before sending down to GPU. This can be disabled by " +
        "setting it to 0. Note that combine also will not go over the " +
        "spark.cudf.sql.reader.batchSizeRows or spark.cudf.sql.reader.batchSizeBytes " +
        s"limits. DEPRECATED: use $READER_MULTITHREADED_COMBINE_THRESHOLD instead.")
      .bytesConf(ByteUnit.BYTE)
      .createOptional

  val PARQUET_MULTITHREADED_COMBINE_WAIT_TIME =
    conf("spark.cudf.sql.format.parquet.multithreaded.combine.waitTime")
  ...
  val PARQUET_MULTITHREADED_READ_KEEP_ORDER =
    conf("spark.cudf.sql.format.parquet.multithreaded.read.keepOrder")
      .doc("When using the MULTITHREADED reader, if this is set to true we read " +
        "the files in the same order Spark does, otherwise the order may not be the same. " +
  ...
  val CLOUD_SCHEMES = conf("spark.cudf.cloudSchemes")
  ...
  val PARQUET_MULTITHREAD_READ_NUM_THREADS =
    conf("spark.cudf.sql.format.parquet.multiThreadedRead.numThreads")
  ...
  val PARQUET_MULTITHREAD_READ_MAX_NUM_FILES_PARALLEL =
    conf("spark.cudf.sql.format.parquet.multiThreadedRead.maxNumFilesParallel")
  ...
  val ENABLE_PARQUET_READ = conf("spark.cudf.sql.format.parquet.read.enabled")
    .doc("When set to false disables parquet input acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_PARQUET_WRITE = conf("spark.cudf.sql.format.parquet.write.enabled")
    .doc("When set to false disables parquet output acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_ORC = conf("spark.cudf.sql.format.orc.enabled")
    .doc("When set to false disables all orc input and output acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_ORC_READ = conf("spark.cudf.sql.format.orc.read.enabled")
    .doc("When set to false disables orc input acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_ORC_WRITE = conf("spark.cudf.sql.format.orc.write.enabled")
    .doc("When set to false disables orc output acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_ORC_BOOL = conf("spark.cudf.sql.format.orc.write.boolType.enabled")
  ...
  val TEST_ORC_STRIPE_SIZE_ROWS = conf("spark.cudf.sql.test.orc.write.stripeSizeRows")
  ...
  val ENABLE_EXPAND_PREPROJECT = conf("spark.cudf.sql.expandPreproject.enabled")
  ...
  val ENABLE_COALESCE_AFTER_EXPAND = conf("spark.cudf.sql.coalesceAfterExpand.enabled")
  ...
  val ENABLE_ORC_FLOAT_TYPES_TO_STRING =
    conf("spark.cudf.sql.format.orc.floatTypesToString.enable")
  ...
  val ORC_READER_TYPE = conf("spark.cudf.sql.format.orc.reader.type")
    .doc("Sets the ORC reader type. We support different types that are optimized for " +
      ...
      "to the GPU separately. This allows the CPU to keep reading while GPU is also doing work. " +
      s"See $MULTITHREAD_READ_NUM_THREADS and " +
      "spark.cudf.sql.format.orc.multiThreadedRead.maxNumFilesParallel to control " +
      "the number of threads and amount of memory used. " +
      "By default this is set to AUTO so we select the reader we think is best. This will " +
      "either be the COALESCING or the MULTITHREADED based on whether we think the file is " +
      "in the cloud. See spark.cudf.cloudSchemes.")
  ...
  val ORC_MULTITHREAD_READ_NUM_THREADS =
    conf("spark.cudf.sql.format.orc.multiThreadedRead.numThreads")
      .doc("The maximum number of threads, on the executor, to use for reading small " +
  ...
  val ORC_MULTITHREAD_READ_MAX_NUM_FILES_PARALLEL =
    conf("spark.cudf.sql.format.orc.multiThreadedRead.maxNumFilesParallel")
      .doc("A limit on the maximum number of files per task processed in parallel on the CPU " +
        "side before the file is sent to the GPU. This affects the amount of host memory used " +
        "when reading the files in parallel. Used with MULTITHREADED reader, see " +
        s"$ORC_READER_TYPE.")
  ...
  val ENABLE_CSV = conf("spark.cudf.sql.format.csv.enabled")
    .doc("When set to false disables all csv input and output acceleration. " +
      "(only input is currently supported anyways)")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_CSV_READ = conf("spark.cudf.sql.format.csv.read.enabled")
    .doc("When set to false disables csv input acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_READ_CSV_FLOATS = conf("spark.cudf.sql.csv.read.float.enabled")
    .doc("CSV reading is not 100% compatible when reading floats.")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_READ_CSV_DOUBLES = conf("spark.cudf.sql.csv.read.double.enabled")
    .doc("CSV reading is not 100% compatible when reading doubles.")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_READ_CSV_DECIMALS = conf("spark.cudf.sql.csv.read.decimal.enabled")
    .doc("CSV reading is not 100% compatible when reading decimals.")
    .booleanConf
    .createWithDefault(false)

  val ENABLE_JSON = conf("spark.cudf.sql.format.json.enabled")
    .doc("When set to true enables all json input and output acceleration. " +
      "(only input is currently supported anyways)")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_JSON_READ = conf("spark.cudf.sql.format.json.read.enabled")
    .doc("When set to true enables json input acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_READ_JSON_FLOATS = conf("spark.cudf.sql.json.read.float.enabled")
    .doc("JSON reading is not 100% compatible when reading floats.")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_READ_JSON_DOUBLES = conf("spark.cudf.sql.json.read.double.enabled")
    .doc("JSON reading is not 100% compatible when reading doubles.")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_READ_JSON_DECIMALS = conf("spark.cudf.sql.json.read.decimal.enabled")
  ...
  val ENABLE_READ_JSON_DATE_TIME = conf("spark.cudf.sql.json.read.datetime.enabled")
    .doc("JSON reading is not 100% compatible when reading dates and timestamps.")
    .booleanConf
    .createWithDefault(false)

  val ENABLE_AVRO = conf("spark.cudf.sql.format.avro.enabled")
    .doc("When set to true enables all avro input and output acceleration. " +
      "(only input is currently supported anyways)")
    .booleanConf
    .createWithDefault(false)

  val ENABLE_AVRO_READ = conf("spark.cudf.sql.format.avro.read.enabled")
    .doc("When set to true enables avro input acceleration")
    .booleanConf
    .createWithDefault(false)

  val AVRO_READER_TYPE = conf("spark.cudf.sql.format.avro.reader.type")
    .doc("Sets the Avro reader type. We support different types that are optimized for " +
      ...
      "to the GPU separately. This allows the CPU to keep reading while GPU is also doing work. " +
      s"See $MULTITHREAD_READ_NUM_THREADS and " +
      "spark.cudf.sql.format.avro.multiThreadedRead.maxNumFilesParallel to control " +
      "the number of threads and amount of memory used. " +
      "By default this is set to AUTO so we select the reader we think is best. This will " +
      "either be the COALESCING or the MULTITHREADED based on whether we think the file is " +
      "in the cloud. See spark.cudf.cloudSchemes.")
  ...
  val AVRO_MULTITHREAD_READ_NUM_THREADS =
    conf("spark.cudf.sql.format.avro.multiThreadedRead.numThreads")
  ...
  val AVRO_MULTITHREAD_READ_MAX_NUM_FILES_PARALLEL =
    conf("spark.cudf.sql.format.avro.multiThreadedRead.maxNumFilesParallel")
      .doc("A limit on the maximum number of files per task processed in parallel on the CPU " +
        "side before the file is sent to the GPU. This affects the amount of host memory used " +
        "when reading the files in parallel. Used with MULTITHREADED reader, see " +
        s"$AVRO_READER_TYPE.")
  ...
  val ENABLE_DELTA_WRITE = conf("spark.cudf.sql.format.delta.write.enabled")
      .doc("When set to false disables Delta Lake output acceleration.")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_ICEBERG = conf("spark.cudf.sql.format.iceberg.enabled")
    .doc("When set to false disables all Iceberg acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_ICEBERG_READ = conf("spark.cudf.sql.format.iceberg.read.enabled")
    .doc("When set to false disables Iceberg input acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_ICEBERG_V3 = conf("spark.cudf.sql.format.iceberg.v3.enabled")
  ...
  val VALIDATE_ICEBERG_DELETION_VECTOR_CRC =
    conf("spark.cudf.sql.format.iceberg.deletionVector.crcCheck.enabled")
  ...
  val ENABLE_ICEBERG_WRITE = conf("spark.cudf.sql.format.iceberg.write.enabled")
    .doc("When set to false disables Iceberg write acceleration")
    .booleanConf
    .createWithDefault(true)

  val ENABLE_HIVE_TEXT: ConfEntryWithDefault[Boolean] =
    conf("spark.cudf.sql.format.hive.text.enabled")
  ...
  val ENABLE_HIVE_TEXT_READ: ConfEntryWithDefault[Boolean] =
    conf("spark.cudf.sql.format.hive.text.read.enabled")
  ...
  val ENABLE_HIVE_TEXT_WRITE: ConfEntryWithDefault[Boolean] =
    conf("spark.cudf.sql.format.hive.text.write.enabled")
  ...
  val ENABLE_READ_HIVE_FLOATS = conf("spark.cudf.sql.format.hive.text.read.float.enabled")
      .doc("Hive text file reading is not 100% compatible when reading floats.")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_READ_HIVE_DOUBLES = conf("spark.cudf.sql.format.hive.text.read.double.enabled")
      .doc("Hive text file reading is not 100% compatible when reading doubles.")
      .booleanConf
      .createWithDefault(true)

  val ENABLE_READ_HIVE_DECIMALS = conf("spark.cudf.sql.format.hive.text.read.decimal.enabled")
  ...
  val ENABLE_RANGE_WINDOW_BYTES = conf("spark.cudf.sql.window.range.byte.enabled")
  ...
  val ENABLE_RANGE_WINDOW_SHORT = conf("spark.cudf.sql.window.range.short.enabled")
  ...
  val ENABLE_RANGE_WINDOW_INT = conf("spark.cudf.sql.window.range.int.enabled")
  ...
  val ENABLE_RANGE_WINDOW_LONG = conf("spark.cudf.sql.window.range.long.enabled")
  ...
  val ENABLE_RANGE_WINDOW_FLOAT: ConfEntryWithDefault[Boolean] =
    conf("spark.cudf.sql.window.range.float.enabled")
  ...
  val ENABLE_RANGE_WINDOW_DOUBLE: ConfEntryWithDefault[Boolean] =
    conf("spark.cudf.sql.window.range.double.enabled")
      .doc("When set to false, this disables the range window acceleration for the " +
  ...
  val ENABLE_RANGE_WINDOW_DECIMAL: ConfEntryWithDefault[Boolean] =
    conf("spark.cudf.sql.window.range.decimal.enabled")
    .doc("When set to false, this disables the range window acceleration for the " +
  ...
  val BATCHED_BOUNDED_ROW_WINDOW_MAX: ConfEntryWithDefault[Integer] =
    conf("spark.cudf.sql.window.batched.bounded.row.max")
  ...
  val ENABLE_SINGLE_PASS_PARTIAL_SORT_AGG: ConfEntryWithDefault[Boolean] =
    conf("spark.cudf.sql.agg.singlePassPartialSortEnabled")
  ...
  val SKIP_AGG_PASS_REDUCTION_RATIO = conf("spark.cudf.sql.agg.skipAggPassReductionRatio")
  ...
  val FORCE_SINGLE_PASS_PARTIAL_SORT_AGG: ConfEntryWithDefault[Boolean] =
    conf("spark.cudf.sql.agg.forceSinglePassPartialSort")
  ...
  val ENABLE_REGEXP = conf("spark.cudf.sql.regexp.enabled")
