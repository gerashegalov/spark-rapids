...
object MetaUtils {
  ...
  def buildTableMeta(tableId: Int, compressed: GpuCompressedColumnVector): TableMeta =
    buildTableMeta(
      tableId,
      compressed.getTableBuffer.getLength,
      compressed.getTableMeta.bufferMeta().getByteBuffer,
      compressed.getTableMeta.rowCount())
