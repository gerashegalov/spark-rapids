...
"""Collect compiler deprecation diagnostics from a GitHub Actions build matrix."""
...
def merge_findings(findings):
    ...
    return sorted(merged.values(), key=lambda finding: (finding.path, finding.line, finding.message))
...
def download_logs(api_url, repository, run_id, token, job_pattern):
    ...
    while True:
        result = request_json(
            f"{api_url}/repos/{repository}/actions/runs/{run_id}/jobs?per_page=100&page={page}",
            token,
        )
        ...
        for delay in (0, 1, 2, 4):
            ...
            try:
                payload = request_bytes(
                    f"{api_url}/repos/{repository}/actions/jobs/{job['id']}/logs", token)
...
def render_summary(findings, failures):
    lines = ["## NVIDIA deprecation audit", ""]
    if findings:
        lines.extend([
            f"Found {len(findings)} unique compiler deprecation diagnostic(s).",
            "",
            "| Owner | Location | Deprecated API | Matrix jobs |",
            "| --- | --- | --- | --- |",
        ])
        for finding in findings[:200]:
            location = f"`{finding.path}:{finding.line}`"
            api = finding.origin or finding.message
            jobs = ", ".join(sorted(finding.jobs))
            lines.append(
                f"| {finding.owner} | {location} | `{markdown_escape(api)}` | "
                f"{markdown_escape(jobs)} |"
            )
        if len(findings) > 200:
            lines.extend(["", f"Report truncated; see the raw artifact for all {len(findings)} findings."])
    else:
        lines.append("No compiler deprecation diagnostics were found in the selected matrix jobs.")
    if failures:
        lines.extend(["", "### Incomplete log collection", ""])
...
def emit_annotations(findings):
    for finding in findings[:50]:
        ...
        print(
            f"::warning file={command_property_escape(finding.path)},line={finding.line},"
            f"title=NVIDIA deprecation::{command_escape(message)}"
        )
    if len(findings) > 50:
        print(f"::warning title=NVIDIA deprecation::Only 50 of {len(findings)} findings were annotated")
...
def parse_args(argv):
    ...
    parser.add_argument("--logs-dir", help="Parse local *.log files instead of downloading job logs")
    ...
    try:
        if args.logs_dir:
            logs = {
                ...
                for path in Path(args.logs_dir).glob("*.log")
            }
        else:
